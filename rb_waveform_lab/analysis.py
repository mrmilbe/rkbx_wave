...
from .config import WaveformAnalysisConfig
from .compression import compress_amplitude, normalize_band
...
def _design_filter_bank(sr: int, cfg: WaveformAnalysisConfig):
	...
	# Four true spectral bands: low, lowmid, midhigh, high.
...
def analyze_bands(
	...
	# Apply per-band gains after compression and normalization so they
	# behave as a linear height scaling in the rendered waveform.
...
def analysis_from_rb_waveform(
	...
	The PWV format stores low/mid/high as columns 0/1/2. We map:
	  col 0 -> low
	  col 1 -> lowmid
	  col 2 -> midhigh and high (duplicated)
	"""
	...
	return WaveformAnalysis(
		...
		lowmid=normalized[:, 1].astype(np.float32, copy=False),
		midhigh=normalized[:, 2].astype(np.float32, copy=False),
...
def resample_analysis(analysis: WaveformAnalysis, target_bins: int) -> WaveformAnalysis:
	...
	return WaveformAnalysis(
		...
		lowmid=_downsample_bins_averaged(analysis.lowmid, target_bins),
		midhigh=_downsample_bins_averaged(analysis.midhigh, target_bins),
